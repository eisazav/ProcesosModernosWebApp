...
const TABLE_HEAD = [
  ...
  { id: 'version', label: 'Versión', alignRight: false },
];
...
export default function UserPage() {
  ...
  const handleDeleteDevice = () => {
    if (currentUserId) {
      DevicesApi.deleteDevice(currentUserId).then(() => {
        const indexToDelete = clients.findIndex(item => item._id === currentUserId);
        if (indexToDelete !== -1) {
  ...
  const handleAddSensor = () => {
    if (currentUserId) {
      navigate(`/dashboard/sensors/new/${currentUserId}`)
    }
  };
  ...
  return (
    <>
      <Helmet>
        <title>Dispositivos | EverGreen</title>
      </Helmet>
      ...
      <Container>
        <Stack direction="row" alignItems="center" justifyContent="space-between" mb={5}>
          ...
          <Button variant="contained"
            ...
            onClick={() => navigate(`/dashboard/dispositivos/new`)}
          >
            Nuevo Dispositivo
          </Button>
        </Stack>
              ...
              <Table>
                ...
                <TableBody>
                  {applySortFilter(clients, getComparator(order, orderBy), filterName).map((row) => {
                    const { name, location, type, version, _id } = row
      ...
      >
        ...
        <MenuItem onClick={() => { handleAddSensor() }}>
